                    ...
                    <div class="card-body">
                        <div class="d-flex flex-column align-items-center text-center">
                            ...
                            <div class="circle-img">
                                <img src="{{ asset("storage/{$file->filename}") }}" alt="account" class="rounded-circle" width="150">
                            </div>
                            @else
                            <div class="circle-img">
                                <img src="{{ asset("storage/{$file->filename}") }}" alt="account" class="premium-photo rounded-circle" width="150">
                            </div>
                            @endif
                            <div class="mt-3">
                                <h4>{{$account->name}}</h4>
                                ...
                                <form method="POST" action="{{ route('followers.destroy', $validate->id) }}">
                                    ...
                                    <input type="text" name="profile_id" value="{{ $account->id }}" hidden>
                ...
                <div class="card mb-3">
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                @if($account->id == Auth::user()->id)
                                <button id="destroy" type="button" class="btn-delete btn btn-danger" data-bs-toggle="modal" data-bs-target="#confirmModal{{ $account->id }}">Delete</button>
                                <div class="modal fade" id="confirmModal{{ $account->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                Are you sure you want to delete the user <strong>{{ $account->name }}</strong> ? <br>
                                                <span class="text-danger">This action cannot be undone.</span>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn-profile btn btn-secondary bsColor" data-bs-dismiss="modal">Close</button>
                                                <form id="form" method="POST" action="{{ route('accounts.destroy', $account->id) }}">
                                                    @csrf
                                                    @method("DELETE")
                                ...
                                <a class="btn-profile btn btn-info" href="{{route('accounts.edit', $account->id)}}">Edit</a>
                ...
                <div class="accordion-item">
                    <h2 class="accordion-header" id="flush-headingOne">
                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseOne" aria-expanded="false" aria-controls="flush-collapseOne">
                            Companies ({{\App\Models\Company::where('author_id', '=', $account->id)->count()}})
                        </button>
                    </h2>
                ...
                <div class="accordion-item">
                    <h2 class="accordion-header" id="headingTwo">
                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
                            Posts ({{\App\Models\Post::where('author_id', '=', $account->id)->count()}})
                        </button>
                    </h2>
